...
public final class Triangulation {
    ...
    public static List<int[]> convexPolygonTriangulate(List<Integer> vertexIndices) {
        ...
        for (int i = 2; i < vertexIndicesCount; i++) {
            triangles.add(new int[]{vertexIndices.getFirst(), vertexIndices.get(i), vertexIndices.get(i - 1)});
        }
     ...
     */
    public static <T extends Vector2D> List<int[]> earClippingTriangulate(List<T> vertices) {
     ...
     * @throws IllegalArgumentException if {@code vertices} size is less than 3
     */
    public static <T extends Vector2D> List<int[]> earClippingTriangulate(List<T> vertices, List<Integer> vertexIndices) {
        int vertexIndicesCount = vertexIndices.size();
        checkVertexIndicesCount(vertexIndicesCount);
        ...
        for (boolean hasClippedEars = true; hasClippedEars; ) {
            ...
            for (int i = 1; i < potentialEarsCount - 1; i++) {
                ...
                Vector2D prevVertex = vertices.get(prevVertexIndex);
     ...
     */
    private static <T extends Vector2D> boolean isCounterClockwise(List<T> vertices, List<Integer> vertexIndices) {
        float area = 0;
        int vertexIndicesCount = vertexIndices.size();
